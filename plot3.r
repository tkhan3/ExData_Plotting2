...
final_plot <- ggplot(plot_df,aes(year,Total,fill=type)) + facet_wrap(~type,nrow=1) + geom_bar(stat="identity") + labs(y=expression("Total " * PM[2.5])) + labs(x="Year") + labs(title="Total PM 2.5/Year per Type") +  guides(fill=FALSE) 
